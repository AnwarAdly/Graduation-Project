...
void MUSART_voidInit ( void ){
	...
	u64 LOC_u64Mantissa = ( F_CPU ) / ( 16 * USART1_BAUD_RATE ) ;

	u64 LOC_u64Fraction = ( ( ( F_CPU * 100 ) / ( 16 * USART1_BAUD_RATE )  ) % 100 ) * 16 ;
...
u8 * MUSART1_PtrReadString ( u8 * Copy_ptrString ){
	...
	while( ( LOC_u8DataCome = MUSART1_u8RecChar() ) != 13 ){

		Copy_ptrString[ LOC_Iterator ] = LOC_u8DataCome ;
		LOC_Iterator++;
	}

	Copy_ptrString[LOC_Iterator] = '\0';

	return ( Copy_ptrString );
